...
async function createCompletion(content: string, userId: string) {
  ...
  if (user) {
    if (user.subscription) {
      ...
      } else if (user.subscription.freeUse === 0 && user.subscription.status === 'expired') {
        return "Тестовые запросы исчерпаны! Подписка не активна."
      }
    }
  }
}
...
export const generateMessage = async (
  ...
  try {
    const { content, userId } = req.body; // Function to generate a random key
    ...
    const message = await createCompletion(content, userId);
    const updatedUser = await User.findOne({ machineId: userId })
